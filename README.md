...
- [x] **Günlük görev ekleme de bağlam-farkında hale getirildi (tutarsızlık giderildi):** Önceki oturumda SADECE haftalık hedef eklemeye (`_haftalik_hedef_isle`) "mevcut N hedefin üzerine ekledim" tarzı bağlam-farkında cevap eklenmişti - günlük görev tarafı (`_gunluk_gorev_isle` ve `YENI_GOREV` yolu) unutulmuştu, hâlâ eklemeden önce mevcut sayıya hiç bakmayan SABİT bir Python şablonu kullanıyordu ("Not aldım, bugünkü görevlerin: ..."). Kullanıcı bunu "SLM doğru anlamıyor" sandı ama aslında SLM burada sadece sınıflandırma yapıyor - cevabı yazan statik şablondu, SLM'in hiçbir anlama hatası yoktu. Artık her iki günlük-görev-ekleme yolu da (`_gunluk_gorev_isle`, `YENI_GOREV`) haftalık taraftakiyle AYNI deseni kullanıyor: eklemeden önce o günün mevcut görev sayısını okuyup "Mevcut N görevinin üzerine ekledim, toplam N+1 oldu" diyor.
