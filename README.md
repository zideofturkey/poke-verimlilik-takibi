...
- [x] GunlukGorevler sekmesi bir noktada Google Sheets'in "Tabloya Dönüştür" özelliğiyle bir Tabloya dönüşmüş - bu, gspread'in `append_row` fonksiyonunun bazen sessizce başarısız olmasına sebep oluyordu. Ham Google Sheets API'si ile (values.append) daha güvenilir yazma yapıldığı doğrulandı; ileride bu sekmeye yazma ile ilgili tuhaf/tutarsız hatalar görülürse bu bilinen bir kırılganlık noktasıdır.
